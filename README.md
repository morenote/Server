...
-   [ ] **去中心化**：morenote没有中心服务器，每个设备上都有一个完整的笔记库。你可以选择离线工作，亦可以选择通过服务器与其他人协同工作。
-   [ ] **格式通用**：morenote优先推荐markdown格式化的文档，用户可以直接通过msync下载自己的markdown。
-   [ ] **开源软件**：morenote是开源软件，所有代码公开，不存在后门
-   [ ] **插件主题**：morenote提供丰富的插件和主题供用户使用
-   [ ] **端到端加密**：morenote提供端到端加密的功能，即使是服务器的管理员也无法偷窥用户的数据
-   [ ] **笔记数据加密**：允许使用加密算法保护笔记数据
-   [ ] **更多编辑器**：morenote适配了很多第三方开源的编辑器，你可以选择你喜欢的
-   [ ] **本地工具编辑**：使用msync下载笔记数据到本地，使用第三方编辑器修改，然后同步
-   [ ] **数据兼容**：与Leanote笔记的API兼容，可以直接使用leanote的桌面软件或移动端软件登录
-   [ ] **后台管理**：更强大的托管后台
-   [ ] **API接口**：morenote开发全部API接口，方便定制功能和插件
