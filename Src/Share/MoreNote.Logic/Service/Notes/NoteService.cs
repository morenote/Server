...
namespace MoreNote.Logic.Service.Notes
{
    public class NoteService
    {
          ...
          bool? isBlog)
        {
           ...
           .Where(b => b.UserId == userId && b.NotebookId == notebookId && b.IsTrash == isTrash && b.IsDeleted == false).OrderBy(s => s.UpdatedTime).Skip(skipNum).Take(pageSize).ToList();
                    break;

                case "PublicTime":
                    result = dataContext.Note
           .Where(b => b.UserId == userId && b.NotebookId == notebookId && b.IsTrash == isTrash && b.IsDeleted == false).OrderBy(s => s.PublicTime).Skip(skipNum).Take(pageSize).ToList();
                    break;

                case "CreatedTime":
                    result = dataContext.Note
           .Where(b => b.UserId == userId && b.NotebookId == notebookId && b.IsTrash == isTrash && b.IsDeleted == false).OrderBy(s => s.CreatedTime).Skip(skipNum).Take(pageSize).ToList();
                    break;

                case "Title":
                    result = dataContext.Note
                .Where(b => b.UserId == userId && b.NotebookId == notebookId && b.IsTrash == isTrash && b.IsDeleted == false).OrderBy(s => s.Title).Skip(skipNum).Take(pageSize).ToList();
                    break;
          ...
          bool? isBlog)
        {
            ...
            switch (sortField)
            {
                case "UpdatedTime":
                    result = dataContext.Note
           .Where(b => b.UserId == userId && b.IsTrash == isTrash && b.IsDeleted == false).OrderBy(s => s.UpdatedTime).Skip(skipNum).Take(pageSize).ToList();
                    break;

                case "PublicTime":
                    result = dataContext.Note
           .Where(b => b.UserId == userId && b.IsTrash == isTrash && b.IsDeleted == false).OrderBy(s => s.PublicTime).Skip(skipNum).Take(pageSize).ToList();
                    break;

                case "CreatedTime":
                    result = dataContext.Note
           .Where(b => b.UserId == userId && b.IsTrash == isTrash && b.IsDeleted == false).OrderBy(s => s.CreatedTime).Skip(skipNum).Take(pageSize).ToList();
                    break;

                case "Title":
                    result = dataContext.Note
                .Where(b => b.UserId == userId && b.IsTrash == isTrash && b.IsDeleted == false).OrderBy(s => s.Title).Skip(skipNum).Take(pageSize).ToList();
                    break;
        ...
        {
            var result = dataContext.Note.Where(b => b.NotebookId == notebookId).OrderBy(x => x.Title).ToList();
            return result;
        }
        ...
        {
            dataContext.Note.Where(a => a.Id == noteId)
