	...
	if(typeof aimgcount == 'object' && $('imagelistthumb_' + pid)) {
		for(pid in aimgcount) {
			...
			for(i = 0;i < aimgcount[pid].length;i++) {
				...
				if(fid) {
					imagelist += '<div id="pattimg_' + aimgcount[pid][i] + '_menu" class="tip tip_4" style="display: none;"><div class="tip_horn"></div><div class="tip_c"><a href="http://47.100.112.22/demo/manga/static/js/forum.php?mod=viewthread&tid=283" class="xi2" onclick="showWindow(\'setcover' + aimgcount[pid][i] + '\', this.href)">设为封面</a></div></div>';
				}
...
function parsetag(pid) {
	...
	var str = $('postmessage_'+pid).innerHTML.replace(/(^|>)([^<]+)(?=<|$)/ig, function($1, $2, $3, $4) {
		...
		$('postmessage_'+pid).innerHTML = str.replace(/<h_ (\d+)>/ig, function($1, $2) {
			return '<span href=\"http://47.100.112.22/demo/manga/static/js/forum.php?mod=viewthread&tid=282\" onclick=\"tagshow(event)\" class=\"t_tag\">' + tagfindarray[$2] + '</span>';
	    	});
	}
}
...
function fastpostappendreply() {
	if($('fastpostrefresh') != null) {
		...
		if($('fastpostrefresh').checked) {
			location.href = 'http://47.100.112.22/demo/manga/static/js/forum.php?mod=viewthread&tid=281'+tid+'&goto=lastpost&random=' + Math.random() + '#lastpost';
			return;
		}
	}
...
function submitpostpw(pid, tid) {
	...
	safescript('md5_js', function () {
		...
		if(!tid) {
			...
			location.href = 'http://47.100.112.22/demo/manga/static/js/forum.php?mod=viewthread&tid=280'+tid;
		}
	}, 100, 50);
}
...
function vmessage() {
	...
	$('vreplysubmit').onmouseover = function() {
		if($('vmessage').value != vf_tips) {
			ajaxget('http://47.100.112.22/demo/manga/static/js/forum.php?mod=viewthread&tid=278', 'vfastpostseccheck');
